    ...
    public partial class Gericht : Form
    {
        ...
        private void SpeichereGerichteInDatenbank()
        {
            foreach (var eintrag in gerichte)
            {
                ...
                foreach (var Gerichte in eintrag.Value)
                {

                    string checkQuery = "SELECT COUNT(*) FROM Rezepte WHERE Gerichte = @Gerichte AND Optionen = @optionen";
                    using (var checkCommand = new SqliteCommand(checkQuery, connection))
                    {
                        checkCommand.Parameters.AddWithValue("@Gerichte", Gerichte);
